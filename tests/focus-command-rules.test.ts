...
describe("Focus Command deterministic gameplay rules", () => {
  ...
  it("returns only due revision topics and measures subject capture from normalized revision progress", () => {
    const state = stateWithToday();
    const today = toLocalDate(new Date().toISOString(), state.profile.timezone);
    ...
    expect(getSubjectCapture(state)).toEqual([{ subject: "Physics", completed: 1, total: 3, capture: 4 / 9, active: 0, planned: 0, seedSown: 1, emerging: 1, developing: 0, reinforcing: 0, consolidating: 0, matured: 1 }]);
  });
