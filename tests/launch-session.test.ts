...
describe("Launch-only lifecycle guard", () => {
  ...
  it("keeps the fire sound bound to the full fire phase and the quote cue bound to the first visible quote frame", () => {
    ...
    expect(getLaunchFireSoundStopDelay(false)).toBeLessThan(getLaunchQuoteVisibleDelay(false));
    ...
    expect(getLaunchFireSoundStopDelay(true)).toBeLessThan(getLaunchQuoteVisibleDelay(true));
