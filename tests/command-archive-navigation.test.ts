...
it("keeps truthful revision-topic rows connected only to the established Revision Queue", () => {
...
it("keeps the approved mobile archive spacing and readable two-column metric grid", () => {
  ...
  expect(archiveSource).toContain("lifetimeReviewEntry: { marginBottom: 14 }");
  expect(archiveSource).toContain("comparisonReviewEntry: { marginTop: 14 }");
