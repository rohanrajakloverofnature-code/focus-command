...
The app process opens into the existing Home state underneath a temporary, viewport-bound overlay; no tab, stack, layout, mission, or dashboard composition changes. The overlay begins with a restrained dark veil, then develops a dense orange fire bed from the bottom edge. Four lightweight animated SVG flame clusters create layered tongues of fire: a near opaque yellow core along the bottom 20–30% of the screen, translucent orange body through the lower half, and sparse, low-opacity red tips that never exceed 50% of the viewport height. A fixed small ember and smoke field drifts upward within the same clipped stage. This avoids a video decode or unbounded particle simulation while retaining continuous, non-static motion.

The selected contextual quote sits in the visual centre, above the dense flame bed, with strong white contrast and a compact dark shadow. The timeline is approximately 4.6 seconds: 0–0.3 seconds dark intro; 0.3–1.5 seconds flame rise; 0.8–1.8 seconds quote entrance; 1.5–3.0 seconds focal hold; 3.0–4.0 seconds flame recession; 3.5–4.5 seconds cyan-gold glaze sweep; then a smooth fade that reveals the untouched application. The overlay accepts no touch interaction while active and unmounts when complete, returning the original screen to normal input.
...
The crackling audio is a short bundled asset played only while the overlay is visible and only when the existing sound preference is enabled. It is stopped and disposed when the overlay completes, is interrupted, or unmounts. Reduced-motion preference compresses the sequence into a brief fade rather than running the continuous flames; high-contrast preference keeps the focal quote shadow and dark veil strong. A process-session guard prevents replays during navigation, re-rendering, background resumption, or orientation changes.
