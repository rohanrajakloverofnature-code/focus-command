          ...
          {state.customGraphs.map((graph, index) => (
            <CommandCard key={graph.id} accent={index === 0 ? colors.primary : index === 1 ? colors.success : "#F4C95D"} style={styles.cardStack}>
              <View style={styles.graphHeader}>
                ...
                <FeedbackPressable onPress={() => updateCustomGraph(graph.id, { enabled: !graph.enabled })} style={({ pressed }) => ({ opacity: pressed ? 0.7 : 1, transform: [{ scale: pressed ? 0.97 : 1 }] })}>
              ...
              <View style={styles.metricChoices}>
                {graphMetrics.map((metric) => {
                  ...
                  return (
                    <FeedbackPressable key={metric.metric} onPress={() => toggleMetric(graph.id, metric.metric, metric.label, metric.color)} style={({ pressed }) => [styles.metricChoice, { backgroundColor: active ? `${metric.color}1D` : colors.background, borderColor: active ? metric.color : colors.border, opacity: pressed ? 0.75 : 1, transform: [{ scale: pressed ? 0.97 : 1 }] }]}>
