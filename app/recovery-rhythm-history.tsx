...
import { formatMinutes, getRecoveryDateRange, getRecoverySummary, type RecoveryRangeKind } from "@/lib/recovery-rhythm";

const rangeOptions: Array<{ kind: RecoveryRangeKind; label: string }> = [{ kind: "week", label: "WEEK" }, { kind: "month", label: "MONTH" }, { kind: "lifetime", label: "LIFETIME" }, { kind: "custom", label: "CUSTOM" }];
...
export default function RecoveryRhythmHistoryScreen() {
  ...
  return <ScreenContainer className="px-4" containerClassName="bg-background" edges={["top", "bottom", "left", "right"]}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    {kind === "custom" ? <View style={styles.customRow}><TextInput value={customStart} onChangeText={setCustomStart} placeholder="Start YYYY-MM-DD" placeholderTextColor={colors.muted} style={[styles.dateInput, { borderColor: colors.border, backgroundColor: colors.surface, color: colors.foreground }]} /><TextInput value={customEnd} onChangeText={setCustomEnd} placeholder="End YYYY-MM-DD" placeholderTextColor={colors.muted} style={[styles.dateInput, { borderColor: colors.border, backgroundColor: colors.surface, color: colors.foreground }]} /></View> : null}
    {!customValid ? <CommandCard accent={colors.warning}><Text style={[styles.emptyText, { color: colors.warning }]}>Enter an inclusive custom start and end date.</Text></CommandCard> : <>
      ...
      <View style={styles.grid}><MetricCard accent={colors.success} label="SUCCESS RATIO" value={balance.success.successRatio === null ? "—" : `${Math.round(balance.success.successRatio * 100)}%`} detail={`${balance.success.recordedDays} recorded day${balance.success.recordedDays === 1 ? "" : "s"}`} /><MetricCard accent={colors.primary} label="MISSION POWER" value={String(Math.round(balance.power))} detail={`${balance.missionCount} completed mission${balance.missionCount === 1 ? "" : "s"}`} /><MetricCard accent={colors.warning} label="AVG STRESS" value={summary.averageStress === null ? "—" : `${summary.averageStress}/10`} detail={`${summary.recordedDays.stress} logged day${summary.recordedDays.stress === 1 ? "" : "s"}`} /><MetricCard accent={colors.success} label="AVG SLEEP" value={formatMinutes(summary.averageSleepMinutes)} detail={summary.averageSleepQuality === null ? `${summary.recordedDays.sleep} logged night${summary.recordedDays.sleep === 1 ? "" : "s"}` : `quality ${summary.averageSleepQuality}/5`} /><MetricCard accent={colors.primary} label="AVG SCREEN" value={formatMinutes(summary.averageScreenMinutes)} detail={summary.commonScreenLabel ? `most logged: ${summary.commonScreenLabel}` : `${summary.recordedDays.screen} logged day${summary.recordedDays.screen === 1 ? "" : "s"}`} /><MetricCard accent={colors.primary} label="RECOVERY ACTIONS" value={String(summary.actions.length)} detail={summary.afterActionChange === null ? "before/after optional" : `avg reported easing ${summary.afterActionChange}/10`} /></View>
      <CommandCard accent={colors.warning} style={styles.insightCard}><Text style={[styles.insightTitle, { color: colors.foreground }]}>Neutral recovery note</Text><Text style={[styles.insightText, { color: colors.muted }]}>{summary.actions.length ? `You recorded ${summary.actions.length} action${summary.actions.length === 1 ? "" : "s"} in this period. Before/after ratings are personal observations, not proof that one action caused a change.` : "Add an optional response after a stressor to build a private record of what felt useful to you."}</Text>{summary.totalNapMinutes ? <StatusPill label={`NAPS · ${formatMinutes(summary.totalNapMinutes)}`} tone="primary" icon="timer" /> : null}</CommandCard>
    </>}
