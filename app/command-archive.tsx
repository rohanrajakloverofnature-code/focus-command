...
function ArchiveYearView({ year, years, onSelectYear, onOpenMonth, onOpenTopics, onOpenLifetimeTopics, onOpenComparison, onOpenCommandBalance, lifetimeWindows, activeLifetimeWindow, onSelectLifetimeWindow, lifetimeSubjects, activeLifetimeSubject, onSelectLifetimeSubject }: { year: MonthlyCommandArchiveYear; years: MonthlyCommandArchiveYear[]; onSelectYear: (year: number) => void; onOpenMonth: (key: string) => void; onOpenTopics: () => void; onOpenLifetimeTopics: () => void; onOpenComparison: () => void; onOpenCommandBalance: () => void; lifetimeWindows: ReturnType<typeof getMonthlyArchiveLifetimeWindows>; activeLifetimeWindow: number; onSelectLifetimeWindow: (index: number) => void; lifetimeSubjects: string[]; activeLifetimeSubject: string; onSelectLifetimeSubject: (subject: string) => void }) {
  const colors = useColors();
  ...
  return <FlatList
    ...
    ListHeaderComponent={<>
      ...
      <TapFeedback onPress={onOpenCommandBalance} accessibilityLabel="Open Recovery and Command Balance overview">
        <View style={[styles.reviewEntry, styles.lifetimeReviewEntry, { borderColor: `${colors.success}66`, backgroundColor: `${colors.success}12` }]}>
          <View style={styles.reviewEntryCopy}><Text style={[styles.subjectTitle, { color: colors.foreground }]}>Command Balance</Text><Text style={[styles.detail, { color: colors.muted }]}>View private stress, sleep, screen time, Success Ratio, and Mission Power with Week, Month, Lifetime, or Custom ranges.</Text></View>
          <Text style={[styles.reviewEntryAction, { color: colors.success }]}>OPEN</Text>
        </View>
      </TapFeedback>
