...
function MonthMetric({ label, value, accent }: { label: string; value: string; accent: string }) {
  const colors = useColors();
  return <View style={[styles.monthMetric, { borderColor: `${accent}66`, backgroundColor: colors.background }]}>
...
function ArchiveYearView({ year, years, onSelectYear, onOpenMonth }: { year: MonthlyCommandArchiveYear; years: MonthlyCommandArchiveYear[]; onSelectYear: (year: number) => void; onOpenMonth: (key: string) => void }) {
  const colors = useColors();
...
function ArchiveMonthView({ month, selectedMetric, onSelectMetric, onBack }: { month: MonthlyCommandArchiveMonth; selectedMetric: MonthlyArchiveMetric; onSelectMetric: (metric: MonthlyArchiveMetric) => void; onBack: () => void }) {
  const colors = useColors();
  ...
  return <FlatList
    ...
    renderItem={({ item }) => <View style={[styles.subjectRow, { borderColor: colors.border }]}>
    ...
    ListHeaderComponent={<>
      ...
      <View style={styles.monthMetrics}>
        <MonthMetric label="XP EARNED" value={`${Math.round(month.xpEarned)}`} accent="#F4C95D" />
        <MonthMetric label="GOLD EARNED" value={`${Math.round(month.goldEarned)}`} accent="#FFAA4C" />
        <MonthMetric label="INVESTED" value={formatHours(month.investedMs)} accent="#55B7FF" />
        <MonthMetric label="COMPLETED" value={`${month.completedMissions}`} accent="#49D17D" />
      </View>
      <SectionHeader title="Reflection traits" />
      <CommandCard accent="#5DD6C0" style={styles.traitsCard}>
        <View style={styles.monthMetrics}>
...
export default function CommandArchiveScreen() {
  ...
  return <ScreenContainer className="px-4" containerClassName="bg-background">
    ...
    /> : selectedMonth ? <ArchiveMonthView month={selectedMonth} selectedMetric={selectedMetric} onSelectMetric={setSelectedMetric} onBack={() => setSelectedMonthKey(null)} /> : <ArchiveYearView year={activeYear} years={archive.years} onSelectYear={setSelectedYear} onOpenMonth={setSelectedMonthKey} />}
  </ScreenContainer>;
}
