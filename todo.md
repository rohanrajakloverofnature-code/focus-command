...
- [x] Inspect and propose exact Weekly, Monthly, and custom-date Focus Friction filters derived only from existing distraction-log records, preserving all current labels and calculations until approved.
- [x] Re-audit the complete long-session performance path after the published selector and audio safeguards, identify any remaining evidence-based sources of 3–4 minute degradation, and propose only internal fixes that preserve every user-facing behavior until approved.
- [x] Complete repeated whole-app, read-only audit passes from startup through every route: provider/navigation retention, state/reducer/persistence, all screen/component render workloads, timers/animations/media lifecycles, and Focus Friction date semantics; report only verified causes before requesting implementation approval.
- [x] Verify and plan a safe deletion cascade so deleting all missions also removes the linked Focus Friction distraction-log analysis and returns the card to zero, without affecting unrelated records or changing mission-deletion behavior until approved.
- [x] Implement approved Focus Friction Weekly, Monthly, Last 14 Days, and Custom inclusive local-date filters from real distraction logs only, preserving current card meanings and visual system.
...
- [x] Inspect the exact Focus Command mission-start, state, backup, protected cinematic, and performance boundaries; then present a brief, flaw-conscious Shadow Gate implementation plan for approval only.
- [x] Expand the Shadow Gate approval-stage plan with its full library, all user choices, personal-learning evidence, saved-data fields, dashboard/history visibility, management controls, deletion, backup, and performance safeguards; do not implement until explicitly approved.
- [x] Revise the approval-stage Shadow Gate plan to add the Discomfort state, a custom date range for Crossed Gates, explicit lightweight personal-doorway storage, backup/restore coverage, and long-session performance-proof requirements; do not implement until explicitly approved.
- [x] Revise the Shadow Gate approval-stage plan for a 120-plus curated offline doorway library and screenshot-matched premium Colour System defaults, while preserving all existing colour customisation behaviour until explicitly approved.
...
- [x] Publish only the validated Core Principles source, tests, and checklist update to public GitHub main after a separate request and temporary deploy-key authorization; do not start an APK or workflow.

- [x] Push the already validated Core Principles update to public GitHub main using a newly generated one-time deploy key; verify the remote commit and remove the local credential immediately afterward.

- [x] Perform a fresh read-only audit of the reported 3–4 minute slowdown across state subscriptions, long-history derivations, chart/list rendering, timers, media, and persistence; prepare an approval-only optimization plan without modifying app code or saved data.
