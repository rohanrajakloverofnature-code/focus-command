...
export interface Mission {
  ...
  /** Identifies the completion instance that generated this future daily mission. */
...
export interface SrsTopic {
  ...
  /** Links an automatically queued revision to one exact mission completion. */
  completionId?: string;
}
...
export interface Transaction {
  ...
  note: string;
  /** Links a completion-created transaction to one exact mission completion. */
  completionId?: string;
}
...
export interface InventoryItem {
  ...
  active: boolean;
  /** Links a completion-created loot item to one exact mission completion. */
  completionId?: string;
  /** Links the consumption of an older loot item to one exact mission completion. */
  consumedByCompletionId?: string;
}
...
export interface MissionCompletion {
  ...
  allowMultipleDailyCompletions?: boolean;
  /** The next-day daily mission generated by this exact completion, when applicable. */
...
function rebuildComboFromCompletions(state: FocusState): ComboState {
  const baseTier = getComboTiers(state)[0];
  let combo: ComboState = {
    currentTierId: baseTier?.id ?? "combo_base",
    lastActiveDate: null,
    qualifyingStreak: 0,
    missedDays: 0,
  };
...
function getReconciledGoldPowerCarry(progression: ProgressionEvent[]): number {
  const retainedAdjustedPower = progression.reduce((total, event) => total + event.powerAwarded * event.goldMultiplier, 0);
  const retainedGoldAlreadyAwarded = progression.reduce((total, event) => total + event.goldAwarded * 10, 0);
  return Math.max(0, retainedAdjustedPower - retainedGoldAlreadyAwarded);
}

/**
 * Removes one durable completion instance and only the state directly created by
 * that instance. The parent mission remains available as a planned mission when
 * it has no other completion instances.
 */
export function removeCompletedMissionRun(state: FocusState, completionId: string): FocusState {
  const completion = state.missionCompletions.find((candidate) => candidate.id === completionId);
  if (!completion) return state;

  const progressionIds = new Set(
    state.progression
      .filter((event) => event.completionId === completionId || event.id === completion.progressionEventId)
      .map((event) => event.id),
  );
  const reflectionIds = new Set(
    state.reflections
      .filter((reflection) => reflection.completionId === completionId || reflection.id === completion.reflectionId)
      .map((reflection) => reflection.id),
  );
  const legacyPendingRevisionMatches = state.srsTopics.filter((topic) =>
    !topic.completionId &&
    topic.status !== "completed" &&
    topic.missionId === completion.missionId &&
    topic.createdAt === completion.completedAt,
  );
  const revisionIds = new Set([
    ...state.srsTopics.filter((topic) => topic.completionId === completionId && topic.status !== "completed").map((topic) => topic.id),
    ...(legacyPendingRevisionMatches.length === 1 ? [legacyPendingRevisionMatches[0].id] : []),
  ]);
  const legacyLootMatches = state.inventory.filter((item) => !item.completionId && item.acquiredAt === completion.completedAt);
  const inventoryIds = new Set([
    ...state.inventory.filter((item) => item.completionId === completionId).map((item) => item.id),
    ...(legacyLootMatches.length === 1 ? [legacyLootMatches[0].id] : []),
  ]);
  const legacyLootTransactions = state.transactions.filter((transaction) =>
    !transaction.completionId &&
    transaction.type === "loot" &&
    transaction.occurredAt === completion.completedAt,
  );
  const transactionIds = new Set([
    ...state.transactions.filter((transaction) => transaction.completionId === completionId || progressionIds.has(transaction.sourceId ?? "")).map((transaction) => transaction.id),
    ...(legacyLootTransactions.length === 1 ? [legacyLootTransactions[0].id] : []),
  ]);
  const remainingCompletions = state.missionCompletions.filter((candidate) => candidate.id !== completionId);
  const remainingForMission = remainingCompletions
    .filter((candidate) => candidate.missionId === completion.missionId)
  ...
  const nextMissions = state.missions
    .filter((mission) => !generatedMissionIds.has(mission.id))
    .map((mission) => {
      if (mission.id !== completion.missionId) return mission;
      const latestRemaining = remainingForMission.at(-1) ?? null;
      const completionHistory = remainingForMission.map((candidate) => candidate.completedAt);
      const revisionTopicIds = mission.revisionTopicIds.filter((id) => !revisionIds.has(id));
      if (!latestRemaining) {
        return {
          ...mission,
      ...
      return {
        ...mission,
        status: mission.allowMultipleDailyCompletions ? "planned" as MissionStatus : "completed" as MissionStatus,
        startedAt: null,
        pausedAt: null,
        pausedMilliseconds: 0,
        endedAt: latestRemaining.completedAt,
        completedAt: latestRemaining.completedAt,
        progressionEventId: latestRemaining.progressionEventId,
        revisionTopicIds,
        completionHistory,
      };
    });
  const nextProgression = state.progression.filter((event) => !progressionIds.has(event.id));
  const nextState: FocusState = {
    ...state,
    missions: nextMissions,
    missionCompletions: remainingCompletions,
    reflections: state.reflections.filter((reflection) => !reflectionIds.has(reflection.id)),
    srsTopics: state.srsTopics.filter((topic) => !revisionIds.has(topic.id)),
    ...
    inventory: state.inventory
      ...
      .map((item) => item.consumedByCompletionId === completionId || (!item.consumedByCompletionId && item.consumedAt === completion.completedAt && item.effectiveOn === removedCompletionDate)
...
export function FocusCommandProvider({ children }: { children: React.ReactNode }) {
  ...
  const removeMissionCompletion = useCallback((completionId: string) => {
    commit((current) => {
  ...
  const finishMission = useCallback((missionId: string, reflectionDraft: ReflectionDraft) => {
    ...
    commit((current) => {
      ...
      const nextDailyMissionId = completedMission.frequency === "daily" && !completedMission.allowMultipleDailyCompletions ? createId("mission") : null;
      const nextDailyMission = nextDailyMissionId ? {
        ...completedMission,
        id: nextDailyMissionId,
        status: "planned" as MissionStatus,
        createdAt: endedAt,
        ...
        generatedByCompletionId: completionId,
      } : null;
      const completion: MissionCompletion = {
        ...
        allowMultipleDailyCompletions: completedMission.allowMultipleDailyCompletions,
        generatedMissionId: nextDailyMissionId ?? undefined,
      };
      return withQueuedOperation({
        ...current,
        missions: [
